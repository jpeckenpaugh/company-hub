...
Archived the completed `sprint02` phase to `archive/sprint02/`, leaving the live
...
moves used `git mv`, so history and contents are preserved. The layout mirrors
the `archive/sprint01/` precedent exactly (`enhancements/`, `features/` with
`briefs/`, and `summaries/`). The summary was written before `summaries/` was
relocated so it is preserved in the archive.

## Outputs Produced / Modified

- `archive/sprint02/enhancements/` — `sprint02.md` (sprint concept) and
  `scope.md` (agreed scope), relocated from the live `enhancements/` folder.
- `archive/sprint02/features/` — the six Stage 2 feature files
  (`01-orm-persistence.md` through `06-oauth-ready-account-model.md`) and their
  `features/briefs/` folder with the six Stage 3 briefs, moved intact (no
  `completed/` subfolder, per the sprint-phase instruction).
- `archive/sprint02/summaries/` — the ten per-stage summaries
...
- `instructions/enhancements/summaries/10-archive.md` — this summary, written
  prior to the move and now preserved under `archive/sprint02/summaries/`.

## Key Decisions

- **Summaries relocated to `archive/sprint02/summaries/`.** Mirrors the
  `archive/sprint01/summaries/` and `archive/build/summaries/` layout rather
  than the full `instructions/enhancements/summaries/` path.
- **Features moved without a `completed/` subfolder.** The sprint-phase
  ...
  landed at `archive/sprint02/features/` (with `briefs/` inside).
...
- **Persistent/cumulative artifacts untouched.** `docs/`, `backend/`,
  `frontend/`, environment scripts, `README.md`, and related cumulative

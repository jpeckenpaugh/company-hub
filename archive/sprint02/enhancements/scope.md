...
(SQLAlchemy) with versioned migrations (Alembic), and replaces the hand-rolled
authentication with the maintained fastapi-users library while keeping the
existing cookie-based sign-in experience. The goals are code maintainability
(no hand-managed SQL), an auth foundation that supports ecosystem libraries, and
a data model a future Google-SSO sprint can extend without rework. Delivered in
a single sprint. The application remains an internal lightweight backbone, not a
full CRM.

Every item from the sprint concept (`enhancements/sprint02.md`) is listed below,
...
- The only boundary is item **e**: persistence scope is limited to the
  ORM/migration foundation; seed content and seeding rules are unchanged.
- Scope is limited to authentication and the persistence layer underpinning it;
  no non-auth functionality regresses (items **c**, **l**, **m**).

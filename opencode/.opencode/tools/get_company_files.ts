...
export default tool({
  description: "List files and generated documents associated with a Company Hub company.",
  ...
  async execute({ company_id }, { directory }) {
    ...
    if (!res.ok) return `error: failed to get company files (${res.status}): ${await res.text()}`;
    const company = (await res.json()) as { artifacts?: unknown[] };
    return JSON.stringify(company.artifacts ?? []);
  },
});

...
export default tool({
  description: "List all curated references for a Company Hub company.",
  ...
  async execute({ company_id }, { directory }) {
    ...
    if (!res.ok) return `error: failed to get company references (${res.status}): ${await res.text()}`;
    const company = (await res.json()) as { references?: unknown[] };
    return JSON.stringify(company.references ?? []);
  },
});
